...
const input: WvFlowInput = { caseId: submitted.caseId, submittedPackage: submitted, sourceEvidence: [evidence], sourceSnapshots: [snapshot], landWells: [], productionLookups: [], deterministicResults: [noEvidence], evidenceAcquisition: [{ sourceId: "test", required: true, status: "succeeded", evidenceIds: [evidence.evidenceId] }] };
...
describe("WV land Phase 5 typed flow", () => {
  ...
  const fixtureFlow: WvFlowInput = { caseId: fixtureInput.caseId, submittedPackage: fixtureInput, sourceEvidence: [...dep, ...ges], sourceSnapshots: [], landWells: [], productionLookups: [], deterministicResults: [production], evidenceAcquisition: [{ sourceId: "wvdep-oog-rbdms-wells", required: true, status: "succeeded", evidenceIds: dep.map((item) => item.evidenceId) }, { sourceId: "wvges-oilgas-wells", required: true, status: "succeeded", evidenceIds: ges.map((item) => item.evidenceId) }, { sourceId: "wvdep-annual-production", required: true, status: "succeeded", evidenceIds: [] }] };
