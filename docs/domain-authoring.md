...
Use the Markdown conventions generated by GitHub Copilot. The reusable
domain-authoring guidance is in this document and the pstack prompt set under
`docs/prompts/`.
...
domains/my-domain/
  domain.md
  agents/evidence-reviewer.agent.md
...
An agent file combines portable metadata with human-readable instructions:
...
The body should define responsibility, boundaries, procedure, and output
requirements. Keep one primary responsibility per agent. Do not put flow
sequencing or multi-agent coordination in an agent file.
